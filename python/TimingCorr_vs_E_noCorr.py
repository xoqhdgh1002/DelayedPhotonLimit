...
gStyle.SetOptStat(0)
gStyle.SetOptFit(0)
outputDir = '/storage/af/user/qnguyen/DelayedPhoton/CMSSW_8_1_0/src/HiggsAnalysis/DelayedPhotonLimit/2018D/TimingCorrection/'
...
file_data = TFile("/storage/cms/store/group/phys_llp/ZeeTiming/2018/hadd/EGamma_Run2018D.root")
tree_data = file_data.Get("ZeeTiming")

file_MC = TFile("/storage/cms/store/group/phys_llp/ZeeTiming/2018/hadd/DYJetsToLL_M-50_TuneCP5_13TeV-amcatnloFXFX-pythia8.root")
#file_MC = TFile("/mnt/hadoop//store/group/phys_susy/razor/Run2Analysis/EcalTiming/2021May3/hadd/DYJetsToEE_M-50_TuneCP5_13TeV-madgraphMLM-pythia8.root")
...
#E_divide = [40.0, 43.0, 46.0, 49.0, 52.0, 55.0, 58.0, 61.0, 64.0, 67.0, 70.0, 80.0, 84.0, 91.0, 100.0, 115.0, 140.0, 190.0, 1000.0]
E_divide = [70, 80.0, 90.0, 100.0, 115.0, 140.0, 190.0, 1000.0]
N_E_points = len(E_divide) - 1
print("N_E_points = {}".format(N_E_points))
...
for i in range(0, N_E_points):
    ...
    hist_1e_this_data = TH1F("hist_1e_this_data_"+str(i),str(E_low_this)+" GeV < E < "+str(E_high_this)+" GeV; E_{1} (GeV)", 60, -1.5, 1.5)
    hist_2e_this_data = TH1F("hist_2e_this_data_"+str(i),"", 60, -1.5, 1.5)
    ...
    text = "#mu={:.2f}#pm{:.2f}".format(result_1e_this_data[0], result_1e_this_data[1])
    result_text = TLatex(0.6, 0.8, text)
    result_text.SetNDC(1)
    result_text.Draw()
    text2 = "#sigma={:.2f}#pm{:.2f}".format(result_1e_this_data[2], result_1e_this_data[3])
    result_text2 = TLatex(0.6, 0.7, text2)
    result_text2.SetNDC(1)
    result_text2.Draw()
    ...
    hist_1e_this_MC = TH1F("hist_1e_this_MC_"+str(i),str(E_low_this)+"< E <"+str(E_high_this)+"; E_{1} (GeV)", 60, -1.5, 1.5)
    hist_2e_this_MC = TH1F("hist_2e_this_MC_"+str(i),"", 60, -1.5, 1.5)
    ...
    text = "#mu={:.2f}#pm{:.2f}".format(result_1e_this_MC[0], result_1e_this_MC[1])
    result_text = TLatex(0.6, 0.8, text)
    result_text.SetNDC(1)
    result_text.Draw()
    text2 = "#sigma={:.2f}#pm{:.2f}".format(result_1e_this_MC[2], result_1e_this_MC[3])
    result_text2 = TLatex(0.6, 0.7, text2)
    result_text2.SetNDC(1)
    result_text2.Draw()
    ...
    #y_E_sigma_time_diff[i] = y_E_sigma_time_data[i] - y_E_sigma_time_MC[i] 
    ...
    #ey_E_sigma_time_diff[i] = np.sqrt(ey_E_sigma_time_data[i]*ey_E_sigma_time_data[i] + ey_E_sigma_time_MC[i]*ey_E_sigma_time_MC[i]) 
    ...
    hist_1e_this_MC_corr = TH1F("hist_1e_this_MC_corr_"+str(i),str(E_low_this)+"< E <"+str(E_high_this)+"; E_{1} (GeV)", 60, -1.5, 1.5)
    hist_2e_this_MC_corr = TH1F("hist_2e_this_MC_corr_"+str(i),"", 60, -1.5, 1.5)
    ...
    text = "#mu={:.2f}#pm{:.2f}".format(result_1e_this_MC_corr[0], result_1e_this_MC_corr[1])
    result_text = TLatex(0.6, 0.8, text)
    result_text.SetNDC(1)
    result_text.Draw()
    text2 = "#sigma={:.2f}#pm{:.2f}".format(result_1e_this_MC_corr[2], result_1e_this_MC_corr[3])
    result_text2 = TLatex(0.6, 0.7, text2)
    result_text2.SetNDC(1)
    result_text2.Draw()
    myC.SaveAs(outputDir+"/ZeeTiming/iEFit_Zee_"+str(i)+"_time_MC_corr.png")
    hist_2e_this_MC_corr.Draw()
    result_2e_this_MC_corr = singGausFit(hist_2e_this_MC_corr)#, -1.0, 1.0, 0.15, 0.30) 
    ...
    hist_1e_this_MC.SetFillColor(rt.kMagenta-7)
    hist_1e_this_MC.SetLineColor(rt.kMagenta-7)
    hist_1e_this_MC.Scale(1./hist_1e_this_MC.Integral())
    hist_1e_this_MC.Draw("hist")
    hist_1e_this_data.Scale(1./hist_1e_this_data.Integral())
    hist_1e_this_data.SetMarkerStyle(20)
    hist_1e_this_data.SetMarkerSize(1)
    hist_1e_this_data.Draw("P same")
    leg_1e = TLegend(0.6,0.75,0.93,0.89)
    leg_1e.SetBorderSize(0)
    leg_1e.SetTextSize(0.03)
    leg_1e.AddEntry(hist_1e_this_MC, "Z #rightarrow ee MC (raw)", "fl")
    leg_1e.AddEntry(hist_1e_this_data, "Z #rightarrow ee Data", "lep")
    leg_1e.Draw()
    myC.SaveAs(outputDir+"/ZeeTiming/iEFit_Zee_"+str(i)+"_data_vs_MC.png")

    hist_1e_this_MC_corr.SetFillColor(rt.kAzure+1)
    hist_1e_this_MC_corr.SetLineColor(rt.kAzure+1)
    hist_1e_this_MC_corr.Scale(1./hist_1e_this_MC_corr.Integral())
    hist_1e_this_MC_corr.Draw("hist")
    hist_1e_this_data.Draw("P same")
    leg_1e = TLegend(0.6,0.75,0.93,0.89)
    leg_1e.SetBorderSize(0)
    leg_1e.SetTextSize(0.03)
    leg_1e.AddEntry(hist_1e_this_MC_corr, "Z #rightarrow ee MC (corr.)", "fl")
    leg_1e.AddEntry(hist_1e_this_data, "Z #rightarrow ee Data", "lep")
    leg_1e.Draw()
    myC.SaveAs(outputDir+"/ZeeTiming/iEFit_Zee_"+str(i)+"_data_vs_MC_corr.png")
...
gr_E_mean_data.GetXaxis().SetLimits(E_divide[0], E_divide[-1])
...
#gr_E_mean_data.GetYaxis().SetRangeUser(-600,300)
...
myC.SaveAs(outputDir+"/ZeeTiming/TimingShift_Zee_vs_E_Data_vs_MC_2018_noCorr.pdf")
myC.SaveAs(outputDir+"/ZeeTiming/TimingShift_Zee_vs_E_Data_vs_MC_2018_noCorr.png")
myC.SaveAs(outputDir+"/ZeeTiming/TimingShift_Zee_vs_E_Data_vs_MC_2018_noCorr.C")
...
gr_E_sigma_dt_data.GetXaxis().SetLimits(E_divide[0], E_divide[-1])
...
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_dt_vs_E_Data_vs_MC_2018_noCorr.pdf")
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_dt_vs_E_Data_vs_MC_2018_noCorr.png")
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_dt_vs_E_Data_vs_MC_2018_noCorr.C")
...
gr_E_sigma_time_data.GetXaxis().SetLimits(E_divide[0], E_divide[-1])
...
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_time_vs_E_Data_vs_MC_2018_noCorr.pdf")
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_time_vs_E_Data_vs_MC_2018_noCorr.png")
myC.SaveAs(outputDir+"/ZeeTiming/TimingReso_Zee_time_vs_E_Data_vs_MC_2018_noCorr.C")
